...
class GCPBudgetMonitor:
    ...
    def get_current_month_cost(self) -> float:
        ...
        try:
            ...
            logger.info(
                f"Getting current month cost from {month_start.date()} to {month_end.date()}"
            )
    ...
    def get_budget_info(self, monthly_budget_limit: float = 100.0) -> BudgetInfo:
        ...
        try:
            ...
            billing_account_id = (
                billing_info.billing_account_name if billing_info else None
            )
            ...
            budget_utilization = (
                (current_cost / monthly_budget_limit) * 100
            ...
            now = datetime.now()
            last_day_of_month = (now.replace(day=1) + timedelta(days=32)).replace(
                day=1
            ) - timedelta(days=1)
    ...
    def check_cost_alerts(self, budget_info: BudgetInfo) -> List[CostAlert]:
        ...
        try:
            ...
            if (
                budget_info.projected_monthly_cost
                > budget_info.monthly_budget_limit * 1.2
            ):
                alerts.append(
                    CostAlert(
    ...
    def create_budget_alert_policy(
        self, budget_limit: float, alert_threshold: float = 80.0
    ) -> bool:
        """
        ...
        try:
            logger.info(
                f"Creating budget alert policy for ${budget_limit} with {alert_threshold}% threshold"
            )
