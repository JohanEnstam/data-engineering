...
class IGDBDataCollector:
    ...
    def save_raw_data(
        self, data: List[Dict], data_type: str, timestamp: str = None
    ) -> str:
        """
    ...
    def collect_all_data(self, games_limit: int = 1000) -> Dict[str, str]:
        ...
        if platforms:
            saved_files["platforms"] = self.save_raw_data(
                platforms, "platforms", timestamp
            )
        ...
        if release_dates:
            saved_files["release_dates"] = self.save_raw_data(
                release_dates, "release_dates", timestamp
            )
