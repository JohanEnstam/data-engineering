...
class IGDBDataValidator:
    ...
    def validate_games_data(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        if missing_columns:
            validation_results["issues"].append(f"Saknade obligatoriska kolumner: {missing_columns}")
        ...
        if 'id' in df.columns:
            ...
            if duplicate_ids > 0:
                validation_results["issues"].append(f"Duplicerade ID:n: {duplicate_ids}")
                validation_results["validation_passed"] = False
        ...
        if len(high_missing) > 0:
            validation_results["issues"].append(f"Kolumner med >50% missing data: {high_missing.to_dict()}")
        ...
        if 'rating' in df.columns:
            ...
            if invalid_ratings > 0:
                validation_results["issues"].append(f"Ogiltiga rating värden: {invalid_ratings}")
        ...
        if 'release_year' in df.columns:
            ...
            if future_years > 0:
                validation_results["issues"].append(f"Spel med release år > {current_year + 5}: {future_years}")
